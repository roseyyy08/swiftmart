...
<div class="container-fluid py-4">
    <div class="text-center mb-4">
        <h2>SwiftMart <small class="text-muted fs-6">Self-Checkout</small></h2>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-md-7">
            <div class="card mb-3">
                <div class="card-body">
                    <h5 class="card-title">Scan Barcode Produk</h5>
                    <div id="scanner-area" style="width:100%; height:300px; background:#000; border-radius:8px; overflow:hidden;"></div>

                    <div class="input-group mt-3">
                        <input type="text" id="manual-barcode" class="form-control" placeholder="Atau ketik barcode manual...">
            ...
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Member (Opsional)</h5>
                    <div id="member-form" class="input-group">
                        <input type="text" id="member-phone" class="form-control" placeholder="Nomor HP Member">
        ...
        <div class="col-md-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <div id="cart-list">
                        <p class="text-muted text-center py-4" id="cart-empty">Belum ada produk di keranjang</p>
                    </div>
                    <hr>
                    ...
                    <button class="btn btn-success w-100 mt-3" id="btn-checkout" disabled data-bs-toggle="modal" data-bs-target="#paymentModal">
                        Bayar Sekarang
                    </button>
                </div>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="btn-group w-100 mb-3" role="group">
                    ...
                    <label class="btn btn-outline-dark" for="pay-cash">Tunai</label>

                    <input type="radio" class="btn-check" name="payment_method" id="pay-debit" value="debit" autocomplete="off">
                    <label class="btn btn-outline-dark" for="pay-debit">Debit</label>

                    <input type="radio" class="btn-check" name="payment_method" id="pay-qris" value="qris" autocomplete="off">
                    <label class="btn btn-outline-dark" for="pay-qris">QRIS</label>
                </div>

                <div id="qris-dummy" class="text-center d-none">
                    <div style="width:180px; height:180px; margin:0 auto; background:repeating-linear-gradient(45deg, #000, #000 10px, #fff 10px, #fff 20px); border:4px solid #000;"></div>
                    <p class="text-muted mt-2 small">(Simulasi QRIS - bukan QR asli)</p>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-primary" id="btn-confirm-payment">Konfirmasi Bayar</button>
            </div>
        </div>
    </div>
</div>
...
Quagga.init({
    ...
    if (err) {
        console.error(err);
        document.getElementById('scanner-area').innerHTML = '<p class="text-white text-center pt-5">Kamera tidak tersedia, gunakan input manual</p>';
        return;
    }
    Quagga.start();
});

let lastScan = null, lastScanTime = 0;
...
function scanBarcode(barcode) {
    apiPost('{{ route("checkout.scan") }}', { barcode }).then(data => {
        if (data.success) {
...
function updateQty(productId, quantity) {
    apiPost('{{ route("checkout.cart.update") }}', { product_id: productId, quantity }).then(data => {
...
document.getElementById('btn-check-member').addEventListener('click', function () {
    ...
    apiPost('{{ route("checkout.member.check") }}', { phone }).then(data => {
...
document.getElementById('btn-confirm-payment').addEventListener('click', function () {
    ...
    apiPost('{{ route("checkout.process") }}', { payment_method: method }).then(data => {
        if (data.success) {
